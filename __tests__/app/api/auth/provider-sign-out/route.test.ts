...
describe('/api/auth/provider-sign-out', () => {
  ...
  describe('Azure configuration', () => {
    it('should handle Azure logout with session and ID token', async () => {
      ...
      const request = createMockRequest(`${baseUrl}/api/auth/provider-sign-out`);
      const response = await GET(request);

      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('https://login.microsoftonline.com/test-tenant-id/oauth2/v2.0/logout');
      expect(response.headers.get('location')).toContain('id_token_hint=test-id-token');
    ...
    it('should construct Azure logout URL without tenant ID', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('https://login.microsoftonline.com/common/oauth2/v2.0/logout');
    });

    it('should use configured end session endpoint when available', async () => {
      process.env.AZURE_END_SESSION_ENDPOINT = 'https://custom.endpoint.com/logout';
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('https://custom.endpoint.com/logout');
    });
  });
  ...
  describe('Starfleet configuration', () => {
    beforeEach(() => {
      ...
      process.env.STARFLEET_AUTHORIZATION_ENDPOINT = 'https://starfleet.example.com/auth';
      process.env.STARFLEET_TOKEN_ENDPOINT = 'https://starfleet.example.com/token';
      process.env.STARFLEET_REDIRECT_URI = 'http://localhost:3000/api/auth/callback/starfleet';
    ...
    it('should handle Starfleet configuration', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toBe(`${baseUrl}/auth/logged-out`);
    });

    it('should use Starfleet end session endpoint when configured', async () => {
      process.env.STARFLEET_END_SESSION_ENDPOINT = 'https://starfleet.example.com/logout';
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('https://starfleet.example.com/logout');
    });
  });

  describe('Well-known endpoint discovery', () => {
    it('should fetch logout URL from well-known endpoint', async () => {
      process.env.AZURE_WELL_KNOWN_URL = 'https://login.microsoftonline.com/test-tenant/.well-known/openid_configuration';
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://login.microsoftonline.com/test-tenant/.well-known/openid_configuration');
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('https://discovered.logout.url/logout');
    });
    ...
    it('should handle well-known endpoint fetch error', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('[Auth] Error fetching well-known configuration:', expect.any(Error));
  ...
  describe('Background logout', () => {
    it('should handle background logout successfully', async () => {
      ...
      const request = createMockRequest(`${baseUrl}/api/auth/provider-sign-out?background=true`);
      const response = await GET(request);
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('https://login.microsoftonline.com/test-tenant-id/oauth2/v2.0/logout'),
  ...
  describe('ID token handling', () => {
    it('should use ID token from query parameter when session not available', async () => {
      ...
      const request = createMockRequest(`${baseUrl}/api/auth/provider-sign-out?idToken=query-id-token`);
      const response = await GET(request);

      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('id_token_hint=query-id-token');
    });
    ...
    it('should prefer session ID token over query parameter', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('id_token_hint=session-id-token');
    });
    ...
    it('should redirect to logged-out when no ID token available', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toBe(`${baseUrl}/auth/logged-out`);
      expect(console.warn).toHaveBeenCalledWith('[Auth] No ID token available for federated logout');
    });
  });
  ...
  describe('Error handling', () => {
    it('should handle general errors gracefully', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toBe(`${baseUrl}/auth/logged-out`);
      expect(console.error).toHaveBeenCalledWith('[Auth] Error during federated sign-out:', expect.any(Error));
    });
  ...
  describe('Environment variable handling', () => {
    ...
    it('should default to localhost when NEXTAUTH_URL is not set', async () => {
      ...
      expect(response.status).toBe(302);
      expect(response.headers.get('location')).toContain('post_logout_redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fauth%2Flogged-out');
    });
  });
  ...
  describe('Logging', () => {
    it('should log session and token information', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('[Auth] Starting federated sign-out process');
      expect(console.log).toHaveBeenCalledWith('[Auth] Session available:', true);
      expect(console.log).toHaveBeenCalledWith('[Auth] ID Token available:', true);
      expect(console.log).toHaveBeenCalledWith('[Auth] Access Token available:', true);
      expect(console.log).toHaveBeenCalledWith('[Auth] Refresh Token available:', true);
      expect(console.log).toHaveBeenCalledWith('[Auth] Background logout:', false);
    });
    ...
    it('should log logout URL construction', async () => {
      ...
      expect(console.log).toHaveBeenCalledWith('[Auth] Using logout URL:', expect.stringContaining('logout'));
      expect(console.log).toHaveBeenCalledWith('[Auth] Federated logout URL constructed:', expect.any(String));
      expect(console.log).toHaveBeenCalledWith('[Auth] Redirect URL:', expect.stringContaining('/auth/logged-out'));
      expect(console.log).toHaveBeenCalledWith('[Auth] ID token length:', expect.any(Number));
    });
  });
});