...
if (typeof TextDecoder === 'undefined') global.TextDecoder = NodeTextDecoder as unknown as typeof TextDecoder;
...
function buildReader(chunks: (string | Uint8Array)[]) {
	...
	const normalized = chunks.map((c) => (typeof c === 'string' ? enc.encode(c) : c));
	let i = 0;
	return {
		read: jest.fn(async () => {
			if (i < normalized.length) return { value: normalized[i++], done: false } as const;
			return { value: undefined, done: true } as const;
		}),
	};
}

function buildResponse(chunks: (string | Uint8Array)[], headers: Record<string, string> = {}) {
	...
	} as unknown as Response & { body: { getReader: () => ReturnType<typeof buildReader> } };
}
