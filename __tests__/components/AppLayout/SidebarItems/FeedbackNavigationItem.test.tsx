...
jest.mock("@ui5/webcomponents-react", () => ({
  ...
  Button: ({ children, onClick, ...props }: any) => (
    <button data-testid={props["data-testid"] || "button"} onClick={onClick} {...props}>
      {children}
    </button>
  ),
}));
...
describe("FeedbackNavigationItem", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockCreateFeedback.mockResolvedValue({ id: "123", message: "Success" } as any);
  });
  ...
  describe("processFeedback", () => {
    ...
    it("handles errors and calls onError callback", async () => {
      ...
      await expect(processFeedback("test feedback", onSuccess, onError)).rejects.toThrow("API Error");

      expect(onError).toHaveBeenCalledWith(error);
      expect(onSuccess).not.toHaveBeenCalled();
      expect(consoleSpy).toHaveBeenCalledWith("Error submitting feedback:", error);
    ...
    it("handles error without onError callback", async () => {
      ...
      await expect(processFeedback("test feedback")).rejects.toThrow("API Error");
  ...
  describe("FeedbackNavigationItem Component", () => {
    it("renders correctly with all elements", () => {
      ...
      expect(screen.getByTestId("text")).toHaveTextContent("Please provide your feedback here");
    ...
    it("calls validation logic with empty feedback", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Feedback text is empty or only whitespace");
